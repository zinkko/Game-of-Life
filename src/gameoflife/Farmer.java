...
public class Farmer extends JPanel {
    ...
    public int nextGeneration() {
        ...
        for (Cell cell : this.solut) {
            ...
            //System.out.println(neighbours);
                ...
                if (! elossa(neighbours)) {
                    this.hitList.add(cell);
                } else if (!cell.isPopulated() && hedelmallinen(neighbours)) {
                    this.newCells.add(cell);
                }
    ...
    private boolean elossa(int naapurit){
        for (int i : this.survive){
            if (i==naapurit) return true;
        }
        return false;
    }
    
    private boolean hedelmallinen(int naapurit) {
        for (int i : this.born){
            if (i==naapurit) return true;
        }
        return false;
